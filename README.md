...
This separation is intentional: the project is not presented as a new PPO algorithm or a new humanoid model. Its contribution is a reproducible simulation benchmarking and model-mismatch evaluation framework.
...
The benchmark defines a successful episode as:

survival_to_timeout

This means that the robot reaches the configured episode time limit without triggering an early termination.

It should therefore be interpreted as a locomotion survival/stability metric, not as reaching a navigation goal.
...
This demonstrates a clear transition from stable locomotion to severe failure as contact friction is reduced.

The values above are treated as results from a controlled robustness sweep rather than an exact physical failure threshold.

Mass-Model Mismatch

The benchmark can also apply a deterministic multiplier to every rigid body in the H1 articulation while recomputing inertia consistently.
